...
function addMeta(value, id, nonce) {
    ...
    jQuery.post(wckAjaxurl, {
        action: "wck_add_meta" + meta,
        meta: value,
        id: id,
        values: values,
        _ajax_nonce: nonce
...
function showUpdateFormMeta(value, id, element_id, nonce) {
    if (jQuery('#update_container_' + value + '_' + element_id).length == 0) {
        jQuery('#container_' + value).parent().css({
            'opacity': '0.4',
            'position': 'relative'
        }).append('<div id="mb-ajax-loading"></div>');
...
function removeUpdateForm(id) {
    jQuery('html, body').animate({
        scrollTop: jQuery('#' + id).prev().offset().top - 40
    }, 700);
...
function updateMeta(value, id, element_id, nonce) {
    ...
    jQuery.post(wckAjaxurl, {
        ...
        if (response.error) {
            ...
            jQuery('html, body').animate({
                scrollTop: jQuery('#container_' + value + ' #element_' + element_id).offset().top - 40
            }, 700);
    ...
    if (element == '.field-label') {
        if (jQuery("#" + parent + ' ' + element).length != 0) {
            ...
            jQuery("#" + parent + ' ' + element).each(function () {
                if (widest == null)
                    widest = jQuery(this);
                else if (jQuery(this).width() > widest.width())
                    widest = jQuery(this);
            });
